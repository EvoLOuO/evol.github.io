...
function checkCoins() {
    if (totalCoins <= 0) {
        message_container.style.display = 'block'; // 显示金幣不足的提示
        setTimeout(() => {
            message_container.style.display = 'none'; // 2秒后隐藏提示
        }, 2000);
        return false; // 如果金币不足，阻止后续操作
    }
    return true; // 如果金币足够，继续执行
}
...
document.addEventListener("keydown", (e) => {
    ...
    if (code == 'ArrowLeft') {
		...
		if (checkCoins()) {	//
            setTimeout(dropClaw, 500); // 只有金币足够时才触发drop
        }
    }
})
...
function endGame(){
	...
	window.location.href = `results.html?coins=${totalCoins}`;
}

/*function updateCoinCount() {
    const coinCountElement = document.getElementById("coin-count");
    const currentCoins = parseInt(coinCountElement.innerText);
    // 保存金币数值到 sessionStorage
    sessionStorage.setItem("coinCount", currentCoins);
}*/
